  ...
  수정
  1.this.defaults.bAction (line 77 )
  ...
  */
  ...
  JXNONESCROLL.prototype.addEventClick = function() {
    ...
    _this.jxwrap.find('>.jx-unit').bind('click', function(e) {
      ...
      if ( _this.defaults.bAction ) {
        if ( $(this)[0].nodeName.toLowerCase() == 'a' || $(this).find('>*')[0].nodeName.toLowerCase() == 'a' ) {
          if ( $(this).attr('href') != undefined || $(this).find('>*').attr('href') != undefined ) {
            //console.log( 1 , $(this).attr('href')  , $(this).find('>*').attr('href') )
            if ( _this.nCurr != $(this).index() ) {
              //a 태그가 있는데 움직여라고 하면 값이 다를때만 움직이게
              e.preventDefault();
              _this.setMove($this);
            }
          } else{
            //a 태그는 있지만 href 값이 없을때에도 움직이고
            console.log( 2 , $(this).attr('href') != undefined , $(this).find('>*').attr('href') != undefined )
            e.preventDefault();
            _this.setMove($this);
          }
        } else  {
  ...
  JXNONESCROLL.prototype.setMove = function(obj) {
    ...
    //console.log('setMove ', _this.nCurr)
  } //setMove
